...
| 対応サービス | ✅ 主要20以上のAWSサービス | ❌ EC2・RDSのみ |
...
| メッセージング | SQS, SNS | ポリシーで主要操作を拒否 | タグ付与で自動復旧 |
| 分析 | OpenSearch | アクセスポリシーで主要操作を拒否 | タグ付与で自動復旧 |
| ネットワーク | IGW, NAT Gateway, VPC Peering, ElasticIP | **条件付き即時削除 ※1** | — |
| 通知のみ | VPC, Glue, ElastiCache, Workspaces, Kinesis, Step Functions, Secrets Manager, CloudFront | **通知のみ（隔離・削除なし）※2** | — |
...
- Kinesis / Step Functions: AWS のリソースポリシー仕様上、全操作拒否による隔離が表現できないため通知のみ
- Secrets Manager / CloudFront: シークレットや配信の停止は影響が大きすぎるため通知のみ
- Workspaces: サービス特性上ネットワーク隔離が適さないため通知のみ
**※3 IAMリソースの挙動**
...
> **補足：** タグ付与制限が厳密に効くのはリソースポリシー対応サービス（S3 / DynamoDB / SQS / SNS / ECR / OpenSearch）です。SGの差し替えで隔離するサービス（EC2 / RDS など）のタグ付与はIAMで制御されるため、運用上はタグ付与権限を絞ることを推奨します。
...
### 2. デプロイ（グローバルイベント転送スタック）
 
IAM Role / IAM User / CloudFront はAWSのグローバルサービスのため、作成イベントが us-east-1 にのみ配信されます。これらを検知するために、転送用の軽量スタックを us-east-1 にデプロイします（EventBridgeルールのみ・追加費用なし）。
...
> **💡 メインスタックを ap-northeast-1 以外にデプロイした場合**は、`--parameter-overrides CoreRegion=<メインスタックのリージョン>` を追加してください。
...
TagWatchman 本体は **オープンソース（Apache License 2.0）** として無料で公開しています。自由にデプロイ・改変してご利用いただけます。
 
あわせて、実環境への導入をより実践的に支援する **上位版（任意・有料）** を提供しています。本体の利用に上位版は必須ではありません。
 
| プラン | 内容 | 価格 |
|---|---|---|
| 本体（OSS） | エージェント一式・本リポジトリの全機能 | **無料** |
| 上位版 | タグ設計の詳細ガイド + IaC テンプレートセット | [¥2,500（税込）](https://buy.stripe.com/9B67sN0Js1zKe5M0ro3ZK01) |
 
> 上位版は「自社のタグ設計をどう決めるか」「既存環境への安全な導入手順」など、本体だけでは判断が難しい運用ノウハウをまとめたものです。
> 上記リンクからご購入いただけます。⭐ Star も励みになります！
