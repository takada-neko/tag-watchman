...
def _handle_stop_logging(trail_name, principal, region, event_time, detail):
    ...
    subject = f"[TagWatchman] {SEVERITY_CRITICAL}: CloudTrail logging was stopped"
    message = "\n".join([
        "=" * 60,
...
def _handle_delete_trail(trail_name, principal, region, event_time, detail):
    ...
    subject = f"[TagWatchman] {SEVERITY_CRITICAL}: CloudTrail trail was deleted"
    message = "\n".join([
        "=" * 60,
...
def _handle_update_trail(trail_name, principal, region, event_time, detail):
    ...
    subject = f"[TagWatchman] {SEVERITY_WARNING}: CloudTrail configuration was changed"
    message = "\n".join([
        "=" * 60,
...
def _handle_put_event_selectors(trail_name, principal, region, event_time, detail):
    ...
    subject = f"[TagWatchman] {SEVERITY_WARNING}: CloudTrail event selectors were changed"
    message = "\n".join([
        "=" * 60,
...
def _publish(subject: str, message: str):
    if not SNS_TOPIC_ARN:
        ...
        return
    # SNS Subject 制約: ASCII・改行/制御文字なし・100字未満（notifier/deleter と同型ガード）
